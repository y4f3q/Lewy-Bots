...
${uye} isimli üyenin <t:${String(Date.now()).slice(0,10)}:F> tarihinde aşağıda istek veya önerisi belirtilmiştir.`)
      ...
      if(modal.customId == "botsorun") {
        ...
        logKanalı.send({content: `<@935539319614078986>`, embeds: [embed.setDescription(`**Merhaba!** ${ayarlar.serverName} Yönetimi
${uye} isimli üyenin <t:${String(Date.now()).slice(0,10)}:F> tarihinde aşağıda bot sorunu bildirdi.`)
        .addField(`İçerik`, `${text}`)
        ]})
        await modal.deferReply({ ephemeral: true })
        await modal.followUp({content: `Başarıyla bot sorunu Şehira'ya iletilmiştir. Teşekkür Ederiz! ${guild.emojiGöster(emojiler.Onay)}` , ephemeral: true })
      }
...
\` 2. Adım \` Kayıt yapmak için \`.kayıt <@lewy/ID> <İsim> <Yaş>\` komutu ile kayıt işlemine başlarsın orda konuştuğun üyenin cinsiyetini belirleyerek kayıt işlemini gerçekleştirirsin ve **5 Saniye** sonra Sohbet odalarına otomatik olarak üyeyi taşıma işlemi yapar.
