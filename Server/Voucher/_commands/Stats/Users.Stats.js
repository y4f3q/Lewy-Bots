...
\`\`\``).setFooter(`${member.user.tag} • ${voiceChannel.name} • ${yayınSüresi}`, member.user.avatarURL({dynamic: true}))]}).catch(err => {})
  ...
  onRequest: async function (client, message, args) { 
    ...
    let load = await message.reply({content: `${message.guild.emojiGöster("Rand_5477")} | **${uye.user.tag}** isimli kullanıcının verileri yükleniyor. Lütfen bekleyin!`})
